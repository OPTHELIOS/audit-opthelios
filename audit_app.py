...
# --- STYLE CSS DÉFINITIF (Correction bugs affichage) ---
with st.sidebar:
...
# --- 3. AUDIT TECHNIQUE COMPLET (WORD + INITIAL) ---
st.header("🔍 Audit Technique")
# Fusion exhaustive des points
sections = {
    "📄 Documentation & Élec": [
    ...
    "☀️ Capteurs & Toiture": [
        "absence de vannes d'isolement sur tuyauterie capteurs", "Dispositif de traversé de toiture ou de parois adapté", 
        "Supports capteurs conformes", "Raccordement correct des capteurs", "Accès capteurs sécurisé", "Absence de masque proche"
    ],
    "⚖️ Équilibrage & Canalisations": [
        "Dispositif d'équilibrage sur chaque champ capteurs", "Dispositifs d'équilibrages sécurisés et exploitables (lisible)", 
        "Matériaux (tuyauteries) conformes à l'usage"
    ],
    "💧 Hydraulique Solaire": [
        "Circulation capteurs et échangeur dans le bon sens", "Présence de vannes pour raccordement pompe de remplissage", 
        "Présence d'un dégazeur sur la conduite chaud (ALLER)", "Soupape de sécurité présente et conforme à l'installation", 
        "Bidon de récupération avec contrôle de niveau", "Circulateur/pompe solaire sur le froid (RETOUR)", 
        "Présence Clapet anti-retour froid (RETOUR)", "Vannes 3 voies (V3V) fonctionnelles"
    ],
    "🎈 Expansion": [
        "Présence d'un vase d'expansion adapté", "Volume du vase d'expansion suffisant", 
        "Présence d'un dispositif d'isolement et de mise à l'air", "Raccordement du vase sur le froid (RETOUR)", "Pression du vase conforme"
    ],
    "📦 Échangeur & Stockage": [
        "Raccordement de l'échangeur en contre-courant", "Vannes d'isolement en entrée et sortie", "Puissance de l'échangeur suffisante", 
        "Ballons dans local fermé, hors gel", "ouverture de porte permettant le passage", "accès complet aux piquages et bride(s)", 
        "Vannes de vidange et de chasse", "présence d'une prise de température en partie haute", 
        "protection cathodique ballon", "calorifuge du stockage", "Coudes vers le bas (Lyres anti-thermosiphon)"
    ],
    "🚿 Distribution ECS": [
        "Mitigeur présent", "Température maximale de l'ECS respectée", "Raccordement correct du bouclage", 
        "Présence des clapets anti-retour", "Bouclage calorifugé"
    ],
    "📊 Métrologie & Tests": [
        "Manomètre de contrôle circuit solaire", "Débitmètre(s) présent(s)", "Sonde d'ensoleillement bien placée", 
        "Seuil d'éclairement conforme", "Sonde de température capteur bien placée", "Sonde Bas de ballon solaire", 
        "Dispositif de prélèvement du liquide caloporteur", "Compteur volumétrique Eau froide", 
        "Tests d'étanchéité conforme", "Réseau rincé", "Télécontrôleur conforme", "Connexion à distance fonctionnelle",
        "COMPTEURS"
    ]
}
...
for sec, pts in sections.items():
    with st.expander(f"📁 {sec}"):
        for p in pts:
            if p == "COMPTEURS":
                st.markdown('<div class="counter-block"><b>📊 Compteur ESU</b>', unsafe_allow_html=True)
                ca, cb = st.columns(2)
                ca.radio("Présent ?", ["Oui", "Non"], key="p_esu")
                cb.text_input("Index", key="i_esu")
                st.camera_input("Photo ESU", key="ph_esu")
                st.markdown('</div>', unsafe_allow_html=True)
            else:
                st.markdown(f"**{p}**")
...
with col_s1:
    ...
    st.table([
        {"Prio": "🔴 P1", "Action": "Sécurité & Sanitaire (Soupape, Vase, Mitigeur)"},
        {"Prio": "🟠 P2", "Action": "Maintenance (Fluide, Calorifuge, Rinçage)"},
        {"Prio": "🔵 P3", "Action": "Métrologie & Suivi (Sondes, Télégestion)"}
    ])
with col_s2:
    st.slider("Note globale / 10", 0, 10, 5)
    st.text_area("Conclusion technique de l'expert", height=150)
st.markdown('</div>', unsafe_allow_html=True)
